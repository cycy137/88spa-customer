        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {promoItems.map((item) => {
            ...
            return (
              <div 
                key={item.id}
                className="relative bg-white border-2 border-spa-gold rounded-3xl p-6 sm:p-8 shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 group"
              >
                {/* 装饰条：卡片左侧的香槟金高光物理线 */}
                <div className="absolute top-0 left-0 w-full sm:w-1.5 h-1.5 sm:h-full bg-spa-gold" />

                {/* 加购项配图：仅当该服务在 ADDON_IMAGES 中有配图时显示 */}
                {addonImage && (
                  <div className="w-full sm:w-64 flex-shrink-0 overflow-hidden rounded-2xl">
                    <img
                      src={addonImage}
                      alt={displayName}
                      loading="lazy"
